...
def main():
    ...
    if request.method == "POST":
        ...
        connection.close()
        return render_template("main.html", name=message[0], articles=articles, feeds=feeds)
...
def selection():
    ...
    if request.method == "GET":
        cursor.execute("DELETE FROM feed_list WHERE item_id=?", (request.args.get("selected"),))
        articles = sort(cursor.execute("SELECT * FROM feed_list WHERE list_id=?", (id + 5,)))
        ...
        if request.form['selected']:
            if request.form['selected'] == "All":
                ...
                articles = sort(cursor.execute("SELECT * FROM feed_list WHERE item_id=?", (request.form['selected'],)))

    # Send feeds to method to return a list of headers to be displayed
    feeds = get_header(cursor.execute("SELECT * FROM feed_list WHERE list_id=?", ((id + 5),)))

    connection.close()
    return render_template("main.html", articles=articles, feeds=feeds)


@login_required
@app.route("/remove", methods=["POST"])
def remove():
    # Method that will delete a feed from the list
    print("hello")
    id = session['user_id']
...
def pull_text(url):
    ...
    # If the method receives a 0 as value t then it will create a new db entry. If it receives a 1 it will update
    # an existing entry.
    connection = sqlite3.connect('news.db', isolation_level=None)
    cursor = connection.cursor()
    ...
    try:
        ...
        if err.code == 403:
            return render_template("error.html", message="The url threw a Forbidden error and cannot be added")

        else:
            return render_template("error.html", message="The url threw an Error and so cannot be added to your feed")
